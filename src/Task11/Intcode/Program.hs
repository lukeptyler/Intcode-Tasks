module Task11.Intcode.Program where
...
import Task11.Intcode.Types      (State(..), Intcode(..),
                                  Program, ProgramT, ProgramIO)
import Task11.Intcode.Accessors  (opcode,
                                  params, paramsWithWrite,
                                  isRunning, isHalted,
                                  pushInput, pushInputList, popOutput)
import Task11.Intcode.Operations
...
runProgramASCII :: Maybe (Char -> String) -> ProgramIO ()
runProgramASCII mPrintOutput = runProgram inputASCII outputASCII
  where
